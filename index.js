...
exports.load_host_list = function () {
  ...
  client.get('config/domains/host_list').string()
  .then(raw_list => {
    if (raw_list) {
    ...
    else console.log("Something went wrong while reading config/domains/host_list from Etcd");
  });

  client.watch()
    .key('config/domains/host_list')
    .create()
    .then(watcher => {
      watcher
